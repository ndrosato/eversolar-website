  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-[50] w-full transition-all duration-300 ${
        transparent ? (scrolled ? "bg-white shadow-md" : "bg-transparent") : "border-b border-neutral-200 bg-white"
      }`}
    >
      <div className="mx-auto flex h-32 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`text-sm font-medium transition-colors hover:opacity-100 relative z-[52] ${
                  transparent ? (scrolled ? "text-[#1C2C5B]" : "text-white") : "text-[#1C2C5B]"
        ...
        <div className="flex items-center space-x-4 relative z-[51]">
          <Link
            ...
            className={`hidden items-center text-sm font-medium transition-colors md:flex hover:opacity-80 ${
              transparent ? (scrolled ? "text-[#1C2C5B]" : "text-white") : "text-[#1C2C5B]"
            }`}
          >
          ...
          <Button
            className={`hidden md:inline-flex relative z-[52] ${
              transparent && !scrolled ? "bg-[#58A342] hover:bg-[#58A342]/90" : "bg-[#58A342] hover:bg-[#58A342]/90"
            } text-white`}
          ...
          <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
            ...
            <SheetContent side="right" className="w-[300px] bg-[#1C2C5B] text-white sm:w-[400px]">
              ...
              <div className="flex flex-col space-y-6 pt-6">
                ...
                <Button
                  className="w-full bg-[#58A342] hover:bg-[#58A342]/90 text-white"
                  onClick={() => {
